...
class PodcastContextService:
    ...
    async def retrieve_podcast_context(
        ...
        try:
            ...
            context_data = await self._retrieve_context_without_threshold(
                user_id=user_id,
                current_message=context_query,
                top_k=top_k
            )

            logger.info(f"Retrieved {len(context_data)} context entries for user {user_id}")
    ...
    async def _retrieve_context_without_threshold(
        ...
        try:
            ...
            query_embedding = await self.user_context_service.embeddings.aembed_query(current_message)
            ...
            for match in matches:
                if hasattr(match, 'score'):  # Just verify score exists, no threshold check
            ...
            logger.info(f"Podcast context retrieval found {len(context_results)} entries for user {user_id} (top {top_k} without threshold)")
    ...
    async def _aggregate_context_for_podcast(
        ...
        try:
            ...
            unique_plants = list(set(plants_mentioned))[:5]  # Top 5 most mentioned plants
            unique_issues = list(set(care_issues))[:3]  # Top 3 common issues
            unique_recommendations = list(set(recommendations_given))[:3]  # Top 3 recommendations
            unique_preferences = list(set(user_preferences))[:3]  # Top 3 preferences

            # Translate extracted context to Vietnamese using LLM
            translated_plants = await self._translate_to_vietnamese(unique_plants, "plant names")
            translated_issues = await self._translate_to_vietnamese(unique_issues, "plant care issues")
            translated_recommendations = await self._translate_to_vietnamese(unique_recommendations, "care recommendations")
            translated_preferences = await self._translate_to_vietnamese(unique_preferences, "user preferences")

            # Determine overall experience level
            experience_level = self._determine_experience_level(experience_indicators)

            # Log what we extracted for debugging
            logger.info(f"Extracted for user {user_id}: plants={translated_plants}, issues={translated_issues}, experience={experience_level}")
    ...
    async def _translate_to_vietnamese(self, items: List[str], context_type: str) -> List[str]:
        """
...
Vietnamese translations:"""
            ...
            response = await self.llm.ainvoke([{"role": "user", "content": translation_prompt}])
            ...
            if len(vietnamese_items) == len(items):
                logger.info(f"Successfully translated {len(items)} {context_type} to Vietnamese")
                return vietnamese_items
            else:
                logger.warning(f"Translation mismatch for {context_type}: expected {len(items)}, got {len(vietnamese_items)}")
                # Pad or truncate to match original length
                if len(vietnamese_items) < len(items):
                    vietnamese_items.extend(items[len(vietnamese_items):])  # Keep original for missing translations
                else:
                    vietnamese_items = vietnamese_items[:len(items)]  # Truncate if too many
                return vietnamese_items
    ...
    def _extract_plant_names(self, summary_text: str) -> List[str]:
        ...
        english_plant_names = [
            "pothos", "snake plant", "monstera", "fiddle leaf fig", "succulent",
            "cactus", "philodendron", "peace lily", "rubber plant", "spider plant",
            "zz plant", "aloe", "jade plant", "orchid", "fern", "bamboo",
            "dracaena", "croton", "calathea", "anthurium", "ficus", "ivy",
            "tomato", "eggplant", "pepper", "cucumber", "lettuce", "basil",
            "mint", "rosemary", "thyme", "lavender", "geranium", "begonia",
            "impatiens", "petunia", "marigold", "sunflower", "rose", "jasmine"
        ]
    ...
    def _extract_care_issues(self, summary_text: str) -> List[str]:
        ...
        issue_patterns = {
            "overwatering": ["overwater", "too much water", "root rot", "yellowing leaves", "soggy soil"],
            "underwatering": ["underwater", "dry soil", "wilting", "crispy leaves", "dehydrated"],
            "poor drainage": ["drainage", "waterlogged", "standing water", "soggy"],
            "lighting issues": ["light", "lighting", "sun", "shade", "bright", "dark"],
            "pest problems": ["pest", "spider mites", "aphids", "fungus gnats", "mealybugs", "scale"],
            "nutrient deficiency": ["nutrient", "fertilizer", "deficiency", "pale leaves", "stunted growth"],
            "humidity issues": ["humidity", "dry air", "brown tips", "crispy edges"],
            "temperature stress": ["temperature", "cold", "heat", "stress", "shock"],
            "fungal infection": ["fungal", "fungus", "mold", "powdery mildew", "black spot"],
            "bacterial infection": ["bacterial", "bacteria", "soft rot", "bacterial blight"]
        }
    ...
    def _extract_recommendations(self, summary_text: str) -> List[str]:
        ...
        recommendation_patterns = {
            ...
            "Review fertilization routine": ["fertiliz", "phân bón", "bón phân", "nutrient"],
            "Relocate for better lighting": [
                ("light" in summary_lower and ("move" in summary_lower or "relocate" in summary_lower)),
                "ánh sáng" in summary_lower and ("di chuyển" in summary_lower or "chuyển chỗ" in summary_lower)
            ],
        ...
        for recommendation, patterns in recommendation_patterns.items():
            ...
            if any(
                (isinstance(pattern, bool) and pattern) or
                (isinstance(pattern, str) and pattern in summary_lower)
    ...
    def _extract_user_preferences(self, summary_text: str) -> List[str]:
        ...
        preference_patterns = {
            "Prefers low-maintenance plants": [
                "low maintenance", "easy care", "dễ chăm sóc", "ít công sức", "không cần chăm nhiều"
            ],
            "Beginner-friendly options": [
                "beginner", "người mới", "mới bắt đầu", "dễ trồng"
            ],
            "Needs drought-tolerant plants": [
                "travel", "du lịch", "đi xa", "khô hạn", "chịu hạn"
            ],
            "Limited growing space": [
                "apartment", "small space", "chung cư", "không gian nhỏ", "diện tích hẹp"
            ],
            "Pet-safe plants required": [
                ("pet" in summary_lower and "safe" in summary_lower),
                "an toàn cho thú cưng", "không độc cho chó", "không độc cho mèo"
            ],
            "Prefers flowering plants": [
                "flower", "bloom", "hoa", "nở hoa", "ra hoa"
            ],
            "Indoor growing preference": [
                "indoor", "trong nhà", "trong phòng"
            ],
            "Outdoor gardening interest": [
                "outdoor", "garden", "ngoài trời", "sân vườn", "làm vườn"
            ]
        }
    ...
    def _extract_experience_level(self, summary_text: str) -> Optional[str]:
        ...
        # Beginner indicators
        beginner_terms = [
            "beginner", "new to plants", "first plant", "người mới", "mới bắt đầu",
            "lần đầu trồng", "chưa có kinh nghiệm", "không biết", "cần hướng dẫn"
        ]

        # Advanced indicators
        advanced_terms = [
            "experienced", "advanced", "có kinh nghiệm", "thành thạo", "chuyên nghiệp",
            "propagation", "nhân giống", "grafting", "ghép cành"
        ]

        # Intermediate indicators
        intermediate_terms = [
            "intermediate", "some experience", "trung bình", "một chút kinh nghiệm"
        ]
    ...
    def _extract_diagnosis_info(self, summary_text: str) -> Optional[Dict[str, str]]:
        ...
        diagnosis_terms = [
            "diagnos", "identified", "disease", "chẩn đoán", "xác định", "bệnh",
            "nhận dạng", "phân tích", "tình trạng", "triệu chứng", "symptom"
        ]

        if any(term in summary_lower for term in diagnosis_terms):
            has_vietnamese = any(
                term in summary_lower
                for term in ["chẩn đoán", "xác định", "bệnh", "nhận dạng", "phân tích", "tình trạng", "triệu chứng"]
            )
            return {
    ...
    async def get_seasonal_recommendations(
        ...
        if season or weather_info:
            recommendations.append("Điều chỉnh tần suất tưới nước theo thay đổi của mùa")
        ...
        for plant in user_plants:
            ...
            if any(term in plant_lower for term in ["cây mọng nước", "xương rồng", "succulent", "cactus"]):
                recommendations.append(f"Giảm tưới nước cho {plant} trong những tháng lạnh hơn")
            elif any(term in plant_lower for term in ["dương xỉ", "fern"]):
                recommendations.append(f"Tăng độ ẩm cho {plant} trong mùa khô")
            elif any(term in plant_lower for term in ["cây đàn hương", "fiddle leaf fig"]):
